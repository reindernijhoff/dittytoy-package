...
`,j=""+new URL("ditty-worklet-B2w7zjzB.js",import.meta.url).href;function G(o){const n=new Blob([I],{type:"application/javascript"});return URL.createObjectURL(n)}function U(){return j}const W=128*5,$=4,y=0,w=0,T=3,S=0,g=1,V=2,O=3,Y=4,P=5,L=6,N=7,q=8,H=9,A=10,E=11,v=12,k=13;/*! unmute-ios-audio. MIT License. Feross Aboukhadijeh <https://feross.org/opensource> */const M=["auxclick","click","contextmenu","dblclick","keydown","keyup","mousedown","mouseup","touchend"];function z(){const o=window.AudioContext||window.webkitAudioContext;function n(){return navigator.userAgent.match(/(iPod|iPhone|iPad)/)&&navigator.userAgent.match(/AppleWebKit/)}if(!n())return new Promise((u,l)=>l());let t="blocked",e="blocked",s,i,r;const a=new o().sampleRate,c=h(a);function h(u){const l=new ArrayBuffer(10),F=new DataView(l);return F.setUint32(0,u,!0),F.setUint32(4,u,!0),F.setUint16(8,1,!0),`data:audio/wav;base64,UklGRisAAABXQVZFZm10IBAAAAABAAEA${window.btoa(String.fromCharCode(...new Uint8Array(l))).slice(0,13)}AgAZGF0YQcAAACAgICAgICAAAA=`}function f(u,l,F){e==="blocked"&&(e="pending",B()),t==="blocked"&&(t="pending",Promise.all([d(),i.resume()]).then(()=>u()).catch(C=>l(C)))}function d(){return s=document.createElement("audio"),s.setAttribute("x-webkit-airplay","deny"),s.preload="auto",s.loop=!0,s.src=c,s.load(),s.play().then(()=>{t="allowed",D()},()=>{t="blocked",s.pause(),s.removeAttribute("src"),s.load(),s=null})}function B(){return i=new o,r=i.createBufferSource(),r.buffer=i.createBuffer(1,1,22050),r.connect(i.destination),r.start(),i.state==="running"?(e="allowed",D(),!0):(e="blocked",r.disconnect(i.destination),r=null,i.close(),i=null,!1)}function D(){t!=="allowed"||e!=="allowed"||M.forEach(u=>{window.removeEventListener(u,f,{capture:!0,passive:!0})})}return new Promise((u,l)=>{M.forEach(F=>{window.addEventListener(F,f.bind(null,u,l),{capture:!0,passive:!0})})})}z().then(()=>{}).catch(()=>{});class K{constructor(){this.events={}}addListener(n,t){var e;(e=this.events[n])!=null&&e.push(t)||(this.events[n]=[t])}removeListener(n,t){var e;this.events[n]=((e=this.events[n])==null?void 0:e.filter(s=>s!==t))||!1}dispatch(n,t){var e;return(e=this.events[n])==null||e.forEach(s=>s(t)),this}}class Z extends K{constructor(){super(),this._workerData=[],this._volumeWorkers=[],this._structure={},this.paused=!0,this.stopped=!0}postMessage(n,t=[]){this.postMessageToWorklet(n,t),this.postMessageToWorkers(n,t)}postMessageToWorklet(n,t=[]){this._worklet&&this._worklet.port.postMessage(n,t)}postMessageToWorkers(n,t=[]){this._workerData.forEach(e=>e.worker.postMessage(n,t))}postMessageToVolumeWorkers(n,t=[]){this.postMessageToWorklet(n,t),this._volumeWorkers.forEach(e=>e.postMessage(n,t))}setVolume(n){this.postMessageToVolumeWorkers({type:q,amp:n})}setInputParameters(n){this.postMessageToWorkers({type:H,vars:n})}log(n,t=-1){return this.dispatch(V,{message:n,line:t})}logClear(){return this.dispatch(g)}error(n,t){return this.dispatch(O,{message:n,...t})}enableAudioVisualizer(n,t){return this.dispatch(P,{context:n,source:t})}onmessage(n){return this.dispatch(n.data.type,n.data)}async stop(){this.stopped||this.log("[STOP]"),this.postMessageToWorklet({type:g}),this._worklet&&await this._context.suspend(),this.paused=!0,this.stopped=!0,this.dispatch(E)}async pause(){this.log("[PAUSE]"),this.paused||await this._context.suspend().then(()=>{this.paused=!0,this.dispatch(v)})}async resume(n=[],t={},e=!1){this.stopped?await this.play(n,t,e):this.paused&&(this.log("[RESUME]"),this._context.resume().then(()=>{this.paused=!1,this.dispatch(k)}))}terminateWorkers(){this._workerData.length>0&&this.log("[TERMINATE WORKERS]"),this._workerData.forEach(n=>n.worker.terminate()),this._workerData=[],this._volumeWorkers=[]}setupWorkers(n,t,e={},s={},i=!0){this.log("[CREATE WORKERS]"),[...n.loops,...n.filters.filter(a=>a.runAs===y)].forEach(a=>{const c={...a},h=new Worker(G());h.addEventListener("error",f=>{this.error(f.message,this.formatErrorCode(f)),this.stop().then(()=>{h.terminate()})}),h.onmessage=f=>{const d=f.data.type;d===O?(this.error(f.data.message,this.formatErrorCode(f.data)),this.stop().then(()=>{h.terminate()})):d!==S&&this.onmessage(f)},c.buffers=Array.from({length:$},(f,d)=>({data:new Float32Array(W*2),index:d})),c.worker=h,c.channel=new MessageChannel,this._workerData.push(c)}),this._workerData.forEach(a=>{a.in=this._workerData.filter(c=>c.out.nodeType===a.nodeType&&c.out.name===a.name).map(c=>({name:c.name,nodeType:c.nodeType,port:c.channel.port2})),a.worker.postMessage({type:S,vars:e,amp:s,port:a.channel.port1,name:a.name,nodeType:a.nodeType,runAs:a.runAs,buffers:a.buffers,in:a.in,code:t},[a.channel.port1,...a.buffers.map(c=>c.data.buffer),...a.in.map(c=>c.port)])}),this._volumeWorkers=this._workerData.filter(a=>a.in.length>0&&a.in.filter(c=>c.nodeType===w).length>0).map(a=>a.worker)}formatErrorCode(n){const t=n.message;return{data:Object.assign(n,{success:!1,toString:()=>t+`
`+Object.keys(n.e).map(e=>{if(!(this[e]==null||typeof this[e]=="object"))return e+": 	"+this[e]},n.e).filter(Boolean).join(`
`)})}}async compile(n){return await this.stop(),this.terminateWorkers(),this.logClear().log("Analyze code...",0),new Promise((t,e)=>{const s=new Worker(G());s.addEventListener("error",i=>{this.error(i.message,this.formatErrorCode(i)),this.log("Unable to analyze code",1),e(this.formatErrorCode(i),s),s.terminate()}),s.addEventListener("message",i=>{const r=i.data.structure;Object.values(r.loops).length<=0&&(r.loops.push({name:"loop_0",nodeType:w,runAs:y,out:{nodeType:T}}),n=`loop(()=>{${n};
},{name:'loop_0',sync:1e10});`),this._structure=r,this._code=n,this.onmessage(i),t(i),s.terminate()}),s.postMessage({type:Y,code:n})})}async play(n=[],t={},e=!1){if(await this.stop(),this.log("[PLAY]"),this.postMessageToWorklet({type:g}),!this._context){const r=window.AudioContext||window.webkitAudioContext;this._context=new r({sampleRate:44100}),this._source=new AudioBufferSourceNode(this._context),this._source.buffer=this._context.createBuffer(2,44100,44100),this._source.loop=!0,await this._context.audioWorklet.addModule(U()),this._worklet=new AudioWorkletNode(this._context,"ditty-worklet",{numberOfInputs:1,numberOfOutputs:1,outputChannelCount:[2]}),this._worklet.onerror=a=>this.error(a.message,a),await this._source.start(),this._source.connect(this._worklet).connect(this._context.destination),this._worklet.port.onmessage=a=>{this.onmessage(a)},this.enableAudioVisualizer(this._context,this._worklet)}this.setupWorkers(this._structure,this._code,n,t,e);const s=this._workerData.filter(i=>i.out.nodeType===T);this.postMessageToWorklet({type:S,in:s.map(i=>({name:i.name,nodeType:i.nodeType,port:i.channel.port2})),state:this._structure,vars:n,amp:t},s.map(i=>i.channel.port2)),this._context.state==="suspended"&&await this._context.resume().then(()=>{}).catch(i=>this.error(i,i)),this.paused=!1,this.stopped=!1,this.dispatch(A)}}const p=.15;function Q(o){return o=Math.max(0,Math.min(1,o)),o*o*o*(o*(o*6-15)+10)}function x(o,n,t){return t=Math.max(0,Math.min(1,(t-o)/(n-o))),t*t*(3-2*t)}function R(o){o+=.5;const t=Math.floor(o),e=o-t;return t+Q(e/.5)}function _(o,n,t){return o*(1-t)+n*t}class X{constructor(n){this.dittytoy=n,this.initialized=!1,this.loops={},this.bpm=0,this.tick=0,this.sampleRate=0,this.volume=.1,this.canvas=document.getElementById("visualiser"),this.ctx=this.canvas.getContext("2d"),this.dittytoy.addListener(S,({structure:t})=>{this.loops={},this.bpm=t.bpm,this.sampleRate=t.sampleRate,this.tick=0,this.targetTick=0;const e=Math.PI*(3-Math.sqrt(5));t.loops.forEach((s,i)=>{this.loops[s.name]={color:`hsl(${60+i*240/t.loops.length}, 75%, 75%)`,notes:[],volume:.1,rotSpeed:_(.05,.2,Math.random())*(Math.random()>.5?1:-1),a:e*i,speedVariation:_(.5,1,Math.random())}})}),this.dittytoy.addListener(P,({context:t,source:e})=>{const s=t.createAnalyser();e.connect(s),s.fftSize=512,this.freqAnalyser=s,this.freqData=new Uint8Array(256),this.initialized||this.update(),this.initialized=!0}),this.dittytoy.addListener(L,t=>{t.amp&&(t.amp.master&&(this.volume=_(this.volume,Math.sqrt(t.amp.master.volume),.01)),t.amp.loops&&t.amp.loops.forEach(e=>{this.loops[e.name]&&(this.loops[e.name].volume=_(this.loops[e.name].volume,Math.sqrt(e.volume),.01))})),t.state&&(this.targetTick=t.state.tick)}),this.dittytoy.addListener(N,t=>{const e=this.loops[t.loop],s=e.a+R(this.tick)*e.rotSpeed*.1-t.note/64*Math.PI*2;e.notes.push({...t,volume:e.volume**1.5,angle:s})})}update(n=0){window.requestAnimationFrame(c=>this.update(c));const t=this.canvas,e=this.ctx,{width:s,height:i}=t.getBoundingClientRect();this.tick=_(this.tick,this.targetTick,.25),(t.width!==s|0||t.height!==i|0)&&(t.width=s|0,t.height=i|0),e.lineWidth=i/300,e.fillStyle="black",e.fillRect(0,0,t.width,t.height);const r=s/i,a=(c,h)=>[(c*.5+.5*r)*i,(h*.5+.5)*i];this.drawInnerCircle(a,i),Object.values(this.loops).forEach(c=>c.notes=this.drawLines(a,c,r)),this.drawAnalyser(a)}drawLines(n,t,e){const s=[],i=.1*t.speedVariation,r=this.ctx;return r.strokeStyle=t.color,t.notes.forEach(a=>{r.globalAlpha=.25+.75*a.volume;const c=Math.max(1e-4,this.tick-a.tick)*i,h=Math.max(1e-4,this.tick-a.tick-a.duration/2)*i,f=p*2+2*c**.9,d=p*2+2*h**.9,B=a.angle;r.beginPath(),r.moveTo(...n(f*Math.sin(B),f*Math.cos(B))),r.lineTo(...n(d*Math.sin(B),d*Math.cos(B))),d<Math.hypot(1,e)&&s.push(a),r.stroke()}),r.globalAlpha=1,s}drawInnerCircle(n,t){const e=this.ctx;e.fillStyle="white",e.strokeStyle="white";const s=this.tick-Math.floor(this.tick),i=this.tick/4-Math.floor(this.tick/4);e.lineWidth*=.5;const r=10;for(let c=0;c<r;c++){const h=1-(c+i)/r,f=p+((c+i)/r)**.9*2;e.globalAlpha=.125*h,e.beginPath(),e.arc(...n(0,0),f*t/2-this.ctx.lineWidth,0,Math.PI*2),e.stroke()}e.lineWidth*=2,e.globalAlpha=1;const a=_(x(.5,1,s)+x(.5,0,s),1,.95)*p;e.beginPath(),e.arc(...n(0,0),a*t/2-this.ctx.lineWidth*2,0,Math.PI*2),e.fill()}drawAnalyser(n){this.freqAnalyser.getByteFrequencyData(this.freqData);const t=this.ctx,e=R(this.tick)*-.1;for(let s=8;s<this.freqData.length;s+=3){const i=s*Math.PI*2/(this.freqData.length-7)+e,r=p+.2*Math.sqrt(this.freqData[s]/255)*this.volume;t.strokeStyle=`hsl(${60+s*240/(this.freqData.length-7)}, 75%, 75%)`,t.beginPath(),t.moveTo(...n(p*Math.sin(i),p*Math.cos(i))),t.lineTo(...n(Math.sin(i)*r,Math.cos(i)*r)),t.stroke()}}}function J(o){const n=new ArrayBuffer(10),t=new DataView(n);return t.setUint32(0,o,!0),t.setUint32(4,o,!0),t.setUint16(8,1,!0),`data:audio/wav;base64,UklGRisAAABXQVZFZm10IBAAAAABAAEA${window.btoa(String.fromCharCode(...new Uint8Array(n))).slice(0,13)}AgAZGF0YQcAAACAgICAgICAAAA=`}function nn(){const o=document.createElement("audio");return o.setAttribute("x-webkit-airplay","deny"),o.preload="auto",o.loop=!0,o.src=J(44100),o.load(),o}let b=null;function tn(o,n=null,t=null){if(!b)b=nn(),document.body.appendChild(b);else return;o.addListener(A,()=>{try{navigator.mediaSession.playbackState="playing",b.play()}catch{}}),o.addListener(k,()=>{try{navigator.mediaSession.playbackState="playing",b.play()}catch{}}),o.addListener(v,()=>{try{navigator.mediaSession.playbackState="paused",b.pause()}catch{}}),o.addListener(E,()=>{try{navigator.mediaSession.playbackState="paused",b.pause()}catch{}});const e=[["play",async()=>{await o.resume()}],["pause",async()=>{await o.pause()}],["previoustrack",n],["nexttrack",t],["stop",async()=>{await o.pause()}]];for(const[s,i]of e)try{navigator.mediaSession.setActionHandler(s,i)}catch{console.log(`The media session action "${s}" is not supported yet.`)}}function en({title:o,artist:n,album:t,artwork:e}){try{navigator.mediaSession.metadata=new MediaMetadata({title:o,artist:n,album:t,artwork:e.map(({src:s,sizes:i,type:r})=>({src:s,sizes:i,type:r}))})}catch{console.log("The media session metadata is not supported yet.")}}function m(o){return document.getElementById(o)}class sn{constructor(){this.dittytoy=new Z,tn(this.dittytoy,()=>this.prev(),()=>this.next()),new X(this.dittytoy),this.ditties=[],this.ditty=null,this.index=0,this.fetchDitties().then(async()=>{var t;const n=((t=window.location.hash)==null?void 0:t.slice(1))||"24373308b4";await this.fetchDitty(n)}),m("play-button").addEventListener("click",async()=>{this.dittytoy.paused?await this.resume():await this.pause(),this.updateUI()}),this.dittytoy.addListener(S,n=>{const t=n.structure.loops.length,e=n.structure.filters.length,s=t+n.structure.filters.filter(r=>r.runAs===y).length,i=(r,a)=>`${r} ${a}${r!==1?"s":""}`;m("log1").innerText=`${i(t,"loop")}, ${i(e,"filter")} (${i(s,"worker")}, 1 AudioWorklet)`}),this.dittytoy.addListener(L,({state:n})=>{n&&(m("log2").innerText=`▸ tick: ${(n.tick||0).toFixed(3)}, time: ${(n.time||0).toFixed(3)} (${n.bpm.toFixed(0)} bpm)`)}),this.dittytoy.addListener(N,n=>{m("log3").innerText=`♪ tick: ${n.tick.toFixed(3)}, note: ${n.note.toFixed(2)} (${n.loop}.${n.synth})`}),[A,k,v,E].forEach(n=>{this.dittytoy.addListener(n,()=>this.updateUI())})}updateUI(){const n=document.querySelector(".play-button i");this.dittytoy.paused||this.dittytoy.stopped?n.className="fas fa-play":n.className="fas fa-pause",this.ditty&&en({title:this.ditty.title,artist:this.ditty.user_id,album:"Dittytoy",artwork:[{src:`https://dittytoy.net/thumbnail/${this.ditty.object_id}.webp`,sizes:"512x512",type:"image/webp"}]})}async fetchDitty(n){var t;await this.dittytoy.stop(),this.updateUI(),this.ditty=await fetch(`https://dittytoy.net/api/v1/ditty/${n}/`).then(e=>e.json()).then(e=>(m("song-name").innerHTML=`<a href="https://dittytoy.net/ditty/${e.object_id}" target="_blank">${e.title}</a>`,m("artist-name").innerHTML=`By <a href="https://dittytoy.net/user/${e.user_id}" target="_blank">${e.user_id}</a>`,e)),await this.dittytoy.compile(this.ditty.code),this.index=this.ditties.findIndex(e=>e===n)??-1,document.querySelectorAll("#ditties li").forEach(e=>{e.classList.remove("selected")}),(t=document.querySelector(`#ditties li[data-id="${n}"]`))==null||t.classList.add("selected"),window.location.hash=`${n}`}async pause(){await this.dittytoy.pause()}async resume(){await this.dittytoy.resume()}async play(){await this.dittytoy.play()}async playDitty(n){(!this.ditty||this.ditty.object_id!==n)&&await this.fetchDitty(n),await this.play()}prev(){this.index=(this.index-1+this.ditties.length)%this.ditties.length,this.fetchDitty(this.ditties[this.index]).then(()=>{this.play()})}next(){this.index=(this.index+1)%this.ditties.length,this.fetchDitty(this.ditties[this.index]).then(()=>{this.play()})}async fetchDitties(){return fetch("https://dittytoy.net/api/v1/ditty/browse/love/").then(n=>n.json()).then(n=>{const e=m("ditties");for(let s=0;s<Math.min(n.objects.length,40);s++){const i=n.objects[s];e.innerHTML+=`<li data-id="${i.object_id}">${i.title} by ${i.user_id}</li>`,this.ditties[s]=i.object_id}document.querySelectorAll("#ditties li").forEach(s=>{s.addEventListener("click",i=>{this.playDitty(i.target.getAttribute("data-id"))})})})}}new sn;
